...
mc.bootstrap <- function(method.reg=c("LinReg","WLinReg","Deming","WDeming","MDeming", "MMDeming", "PaBa", "PaBaLarge"), X, Y, error.ratio,
                         ...
                         nnested=25, iter.max=30, threshold=0.00000001, NBins=1000000, slope.measure=c("radian","tangent"))
{
	...
	if(method.reg %in% c("WDeming", "MDeming", "MMDeming", "PaBa", "PaBaLarge"))
    {
  ...
  # ----------------------------------------------------------------------------
	...
	if(jackknife==TRUE)
    {
    	...
    	for(i in 1:npoints)
        {
   	    ...
   	    if(method.reg %in% c("LinReg","WLinReg","Deming","MDeming","MMdeming"))
        {
